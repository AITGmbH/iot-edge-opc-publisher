        ...
        public async Task ShowDiagnosticsInfoAsync(CancellationToken ct)
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    Logger.Information($"monitored item settings queue bounded capacity: {diagnosticInfo.MonitoredSettingsQueueCapacity}");
                    Logger.Information($"monitored items queue current items: {diagnosticInfo.MonitoredItemsQueueCount}");
                    Logger.Information($"monitored items settings queue current items: {diagnosticInfo.MonitoredSettingsQueueCount}");
                    ...
                    Logger.Information("---------------------------------");
                    Logger.Information($"monitored event properties queue bounded capacity: {diagnosticInfo.MonitoredPropertiesQueueCapacity}");
                    Logger.Information($"monitored events properties items: {diagnosticInfo.MonitoredPropertiesQueueCount}");
                    Logger.Information("---------------------------------");
                    Logger.Information($"messages sent to IoTHub: {diagnosticInfo.SentMessages}");
                    Logger.Information($"properties sent to IoTHub: {diagnosticInfo.SentProperties}");
                    Logger.Information($"settings sent to IoTHub: {diagnosticInfo.SentSettings}");
                    Logger.Information($"events sent to IoTHub: {diagnosticInfo.SentIoTCEvents}");
